 ...
 * system properties.
 *
 * The `spec` property will be used to filter Specifications, and
 * `example` will filter examples within those Specifications. Because of
 ...
 *
 * Substring regex matching is used, so the simple case of `-Dspec=Timeline` (a
